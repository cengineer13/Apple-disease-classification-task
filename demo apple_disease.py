...
#CONSONANT VARIABLES
SAVE_DIR, SAVE_PREFIX = "data/saved_models", "apple_disease"
...
def run(args): 
    ...
    tfs = T.Compose([T.Resize(img_size), T.ToTensor(), T.Normalize(mean=mean, std=std)])
    ...
    #img for prediction
    example_im_path = "data/test_images/apple_alternariraboltch.jpg"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Apple Leaf Disease Classifier DEMO')
    
    #Add arguments (- va -- option value oladigon, type - qaysi turni olish )
    parser.add_argument("-mn", "--model_name", type=str, default="resnet50", help="AI model nomi")
    parser.add_argument("-mp", "--model_path", type=str, default="data/saved_models/apple_disease_best_model.pth", 
                        help="Path for best saved model")
   # parser.add_argument("-imp", "--img_path", default="data/test_images/apple_alternariraboltch.jpg", help="Path for image")
